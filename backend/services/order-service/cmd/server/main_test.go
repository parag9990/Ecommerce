...
func TestPaymentEventIngressAuthenticatesAndAppliesCapture(t *testing.T) {
	...
	handler := serviceMux(nil, executor, nil, "admin-token-at-least-32-characters", "events-token", slog.New(slog.NewTextHandler(io.Discard, nil)))
...
func TestPaymentEventIngressRejectsMissingAuthorization(t *testing.T) {
	handler := serviceMux(nil, &fakePaymentResultExecutor{}, nil, "admin-token-at-least-32-characters", "events-token", slog.Default())
