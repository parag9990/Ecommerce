...
func (h *Handler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.Handle("GET /api/v1/seller/coupons", h.requireInternalAuth(http.HandlerFunc(h.handleListCoupons)))
	mux.Handle("POST /api/v1/seller/coupons", h.requireInternalAuth(http.HandlerFunc(h.handleCreateCoupon)))
	mux.Handle("PATCH /api/v1/seller/coupons/{coupon_id}", h.requireInternalAuth(http.HandlerFunc(h.handleUpdateCoupon)))
	mux.Handle("POST /api/v1/seller/coupons/{coupon_id}/disable", h.requireInternalAuth(http.HandlerFunc(h.handleDisableCoupon)))
	...
	mux.Handle("GET /api/v1/seller/campaigns", h.requireInternalAuth(http.HandlerFunc(h.handleListCampaigns)))
	mux.Handle("POST /api/v1/seller/campaigns", h.requireInternalAuth(http.HandlerFunc(h.handleCreateCampaign)))
	mux.Handle("PATCH /api/v1/seller/campaigns/{campaign_id}", h.requireInternalAuth(http.HandlerFunc(h.handleUpdateCampaign)))
	mux.Handle("POST /api/v1/seller/campaigns/{campaign_id}/disable", h.requireInternalAuth(http.HandlerFunc(h.handleDisableCampaign)))
