...
const preferencesCollectionName = process.env.NOTIFICATION_MONGO_PREFERENCES_COLLECTION || "notification_preferences";
...
if (notificationDB.getCollectionInfos({ name: preferencesCollectionName }).length === 0) {
  assert.commandWorked(notificationDB.createCollection(preferencesCollectionName, {
  ...
  assert.commandWorked(notificationDB.runCommand({
    collMod: preferencesCollectionName,
    validator: preferenceValidator,
    validationLevel: "strict",
    validationAction: "error"
  }));
...
notificationDB.getCollection(preferencesCollectionName).createIndex(
  { user_id: 1 },
