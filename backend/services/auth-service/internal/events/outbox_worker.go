...
func NewOutboxWorker(repo OutboxRepository, publisher Publisher, cfg OutboxWorkerConfig, logger *slog.Logger) (*OutboxWorker, error) {
	...
	if cfg.Topic == "" {
		return nil, errors.New("auth events topic is required")
	}
	if cfg.BatchSize <= 0 {
		return nil, errors.New("outbox batch size must be greater than zero")
	}
	if cfg.Interval <= 0 {
		return nil, errors.New("outbox interval must be greater than zero")
	}
	if cfg.MaxAttempts <= 0 {
		return nil, errors.New("outbox max attempts must be greater than zero")
	}
	if cfg.InitialBackoff <= 0 {
		return nil, errors.New("outbox initial backoff must be greater than zero")
	}
	if cfg.MaxBackoff < cfg.InitialBackoff {
		return nil, errors.New("outbox max backoff must be greater than or equal to initial backoff")
	}
	if cfg.StaleLockTimeout <= 0 {
		return nil, errors.New("outbox stale lock timeout must be greater than zero")
	}
	if cfg.WorkerID == "" {
		host, _ := os.Hostname()
		if host == "" {
...
func (w *OutboxWorker) Run(ctx context.Context) {
	...
	for {
		if err := w.RunOnce(ctx); err != nil && !errors.Is(err, context.Canceled) {
			w.logger.ErrorContext(ctx, "auth.outbox.run_failed",
...
func (w *OutboxWorker) RunOnce(ctx context.Context) error {
	now := w.clock.Now().UTC()
	staleBefore := now.Add(-w.cfg.StaleLockTimeout)
	events, err := w.repo.LockPendingOutboxEvents(ctx, w.cfg.BatchSize, now, staleBefore, w.cfg.WorkerID)
	if err != nil {
		return err
	}
	...
	for _, event := range events {
		if err := w.publishOne(ctx, event); err != nil {
			w.logger.WarnContext(ctx, "auth.outbox.publish_failed",
...
func (w *OutboxWorker) publishOne(ctx context.Context, event domain.OutboxEvent) error {
	...
	if err == nil {
		now := w.clock.Now().UTC()
		if markErr := w.repo.MarkOutboxEventPublished(ctx, event.EventID, now); markErr != nil {
			return markErr
		}
		w.logger.InfoContext(ctx, "auth.outbox.published",
			slog.String("event_id", event.EventID),
			slog.String("event_type", event.EventType),
	...
	delay := w.backoff(nextAttemptNumber)
	nextAttemptAt := w.clock.Now().UTC().Add(delay)
