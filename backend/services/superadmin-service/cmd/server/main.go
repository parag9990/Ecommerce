...
func main() {
	...
	server := &http.Server{
		Addr:              cfg.HTTPAddr,
		Handler:           transporthttp.NewServeMux(rbacHandler, controlHandler, orderPaymentHandler, sessionVisibilityHandler, settingsHandler, auditLogHandler),
...
func buildUserServiceClient(cfg config.Config, logger logging.Logger) (usecase.UserServiceClient, error) {
	...
	return clients.NewHTTPUserServiceClient(cfg.UserServiceBaseURL, cfg.UserServiceTimeout, logger)
}
...
func buildOrderServiceClient(cfg config.Config, logger logging.Logger) (usecase.OrderServiceClient, error) {
	...
	return clients.NewHTTPOrderServiceClient(cfg.OrderServiceBaseURL, cfg.OrderServiceTimeout, logger)
}
...
func buildPaymentServiceClient(cfg config.Config, logger logging.Logger) (usecase.PaymentServiceClient, error) {
	...
	return clients.NewHTTPPaymentServiceClient(cfg.PaymentServiceBaseURL, cfg.PaymentServiceTimeout, logger)
}
