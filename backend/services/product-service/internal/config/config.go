...
func Load() (Config, error) {
	...
	cfg.Events.KafkaBrokers = csvEnv("PRODUCT_KAFKA_BROKERS", cfg.Events.KafkaBrokers)
...
func (c Config) Validate() error {
	...
	if c.Events.Enabled {
		...
		if strings.EqualFold(strings.TrimSpace(c.Events.Broker), "kafka") && len(c.Events.KafkaBrokers) == 0 {
			return fmt.Errorf("PRODUCT_KAFKA_BROKERS is required when PRODUCT_EVENT_BROKER is kafka")
		}
