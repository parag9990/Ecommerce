...
func RegisterUserRoutes(
	...
	buyer := func(handler http.Handler) http.Handler {
		return auth.Required(rbac.RequireAnyRole("buyer", "seller", "admin", "superadmin")(handler))
	}
	seller := func(handler http.Handler) http.Handler {
		return auth.Required(rbac.RequireAnyRole("seller", "seller_manager", "superadmin")(handler))
	}
